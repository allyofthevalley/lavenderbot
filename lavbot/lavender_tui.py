...
class LavenderTUI(App):
    ...
    async def handle_command(self, chat: RichLog, command: str):
        ...
        if cmd == "/reset":
            ...
            chat.write("  • All memories (lavender_memory/)")
            chat.write("  • All moments (lavender_moments/)")
            chat.write("  • All users, API keys & Discord token (user.db)")
    ...
    async def _do_factory_reset(self, chat: RichLog):
        """Wipe all memories, moments, users, API keys, and Discord token."""
        errors = []

        # 1. Delete lavender_memory/ directory
        mem_dir = "lavender_memory"
        ...
        # 2. Delete lavender_moments/ directory
        mom_dir = "lavender_moments"
        if os.path.exists(mom_dir):
            try:
                shutil.rmtree(mom_dir)
                chat.write(f"Deleted {mom_dir}/")
            except Exception as e:
                errors.append(f"Could not delete {mom_dir}/: {e}")

        # 3. Wipe all settings and users from user.db
        try:
            ...
            with _connect() as conn:
                ...
                conn.commit()
            chat.write("Cleared all users, API keys, Discord token, and Ollama settings from user.db.")
        except Exception as e:
            errors.append(f"Could not clear user.db: {e}")

        # 4. Clear selected user in this session
        self.current_user_id = None
